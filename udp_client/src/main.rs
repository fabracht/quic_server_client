...
impl Client {
    pub fn new(config: &mut Config, peer_address: SocketAddr, udp_socket: UdpSocket, rx: futures::channel::oneshot::Receiver<ClientMessage> ) -> Self {
        let mut scid = [0; quiche::MAX_CONN_ID_LEN];
        SystemRandom::new().fill(&mut scid[..]).unwrap();
        let scid = quiche::ConnectionId::from_ref(&scid);
        let conn =
            quiche::connect(Some("quic.tech"), &scid, peer_address, config).unwrap();
